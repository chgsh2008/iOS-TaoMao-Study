...
-(void)addsubView
{
    ...
    for (int i =0; i<6; i++){//self.marrayAll.count
...
//        }
        ...
        if (i == 0 ) {
            [image setImage:[UIImage imageNamed:@"shop_df_big_.png"]];
            [btnNine addSubview:image];
            title_label.text= @"哈密瓜";
        }
        else if(i == 1)
        {
            [image setImage:[UIImage imageNamed:@"default_01.png"]];
            [btnNine addSubview:image];
            title_label.text= @"葫芦瓜";
        }
        else if(i == 2)
        {
            [image setImage:[UIImage imageNamed:@"default_02.png"]];
            [btnNine addSubview:image];
            title_label.text= @"香蕉";
        }
        else if(i == 3)
        {
            [image setImage:[UIImage imageNamed:@"default_03.png"]];
            [btnNine addSubview:image];
            title_label.text= @"杂粮";
        }
        else if(i == 4)
        ...
        {
            [image setImage:[UIImage imageNamed:@"df_04_.png"]];
            [btnNine addSubview:image];
            title_label.text= @"奇异果";
        }
        title_label.font=[UIFont systemFontOfSize:16];
        title_label.backgroundColor=[UIColor clearColor];
        title_label.textColor =hui2;
        title_label.textAlignment=0;
        //title_label.text= @"xiangjiao";//[[self.marrayAll objectAtIndex:i]objectForKey:@"name"];
        ...
        title_label.tag = btnNine.tag +10;
        titleDetail.text=@"好吃，美味，营养丰富";//[self.marrayAll objectAtIndex:i]objectForKey:@"description"];
...
-(void)btnNine:(id)sender
{
    ...
    NSLog(@"%d",btn.tag);
    UILabel *title = (UILabel *)[btn viewWithTag: (btn.tag +10)];
    NSString *string1 = title.text;
    string1 = [string1 stringByAppendingString:@"信息"];
...
//            addWithID:[NSString stringWithFormat:@"%@",[[self.marrayAll objectAtIndex:btn.tag-1000]objectForKey:@"id"]]];
    TMSecondClassViewController *secondV = [[TMSecondClassViewController alloc]initWithTitle:string1 addWithID:@""];
